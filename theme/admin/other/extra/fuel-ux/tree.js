 ...
 *
 *
 * REWRITTEN TO WORK WITH BS3 AND FONT-AWESOME!!!
 *
 */
    ...
    Tree.prototype = {
        ...
        selectItem: function (el) {
            ...
            if (this.options.multiSelect) {
                ...
                $all.removeClass('tree-selected')
                    .find('i:first').removeClass('fa-check').addClass('tree-dot');
                data.push($el.data());
            }

            if($el.hasClass('tree-selected')) {
                $el.removeClass('tree-selected');
                $el.find('i:first').removeClass('fa-check').addClass('tree-dot');
            } else {
                $el.addClass ('tree-selected');
                $el.find('i:first').removeClass('tree-dot').addClass('fa-check').addClass('fa');
                if (this.options.multiSelect) {
                    data.push( $el.data() );
                }
            }
        ...
        selectFolder: function (el) {
            ...
            if($el.find('.fa-folder').length) {
                ...
                $par.find('.fa-folder:eq(0)')
                    .removeClass('fa-folder')
                    .addClass('fa-folder-open').addClass('fa');
                ...
                $par.find('.fa-folder-open:eq(0)')
                    .removeClass('fa-folder-open')
                    .addClass('fa-folder').addClass('fa');
