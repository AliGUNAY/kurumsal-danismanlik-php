								...
								<div class="panel panel-default">
										...
										<table class="table table-striped table-hover table-bordered">
											...
											{foreach from=$products item=don}
												<tr id="item-{$don.id}">
													<td class="text-center"><button type="button" data-name="{$don.seo_title}" data-database="page/announcements/status" data-id="{$don.id}" data-status="{$don.status}" class="btn {if $don.status eq 0}btn-primary{else}btn-danger{/if} btn-xs status"><i class="fa fa-star" aria-hidden="true"></i></button></td>
													<td class="text-center"><button type="button" class="btn btn-warning btn-xs "><i class="fa fa-arrows" aria-hidden="true"></i></button></td>
													<td>{$don.id}</td>
													<td>{$don.seo_title}</td>
													<td>
														<a href="index.php?page=page/announcements/edit&id={$don.id}"><span class="label label-info"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Düzenle</span></a>
														<a class="divtikla" href="index.php?page=page/announcements/delete&id={$don.id}"><span class="label label-danger"><i class="fa fa-times" aria-hidden="true"></i> Sil</span></a>
													</td>
												</tr>
											{/foreach}
											{else}
		...
		{if $categorys eq 1}	
			<div id="long" class="modal fade" tabindex="-1" data-replace="true" data-width="760" style="display: none;">
				...
				<div class="modal-header">
					...
					<h4 class="modal-title">{$homepage.title} Ekle</h4>
				</div>
