{include file="header.tpl"}
 <div id="main-wrapper">
	<aside>
		...
		<div id="content">
			<div class="row">
				<form class="verikaydet" action="index.php?page=page/services" method="post" enctype="multipart/form-data">
							...
							<div class="panel-body">
								<div class="panel panel-default">
										<table class="table table-striped">
											...
											<tbody id="sortablecategory" data-page="page/services/category/sort">
											{if $category eq TRUE}
											{foreach from=$category key=k item=categorydon}
												{if $categorydon.ustkategori eq 0}
												<tr id="item-{$categorydon.id}">
													<td class="text-center"><button type="button" data-name="{$categorydon.seo_title}" data-database="page/services/category/status" data-id="{$categorydon.id}" data-status="{$categorydon.status}" class="btn {if $categorydon.status eq 0}btn-primary{else}btn-danger{/if} btn-xs status"><i class="fa fa-star" aria-hidden="true"></i></button></td>
													...
													<td>
														<a href="index.php?page=page/services/category/edit&id={$categorydon.id}"><span class="label label-info"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Düzenle</span></a>
														<a class="divtikla" href="index.php?page=page/services/category/delete&id={$categorydon.id}"><span class="label label-danger"><i class="fa fa-times" aria-hidden="true"></i> Sil</span></a>
													</td>
												</tr>
												{/if}
											{/foreach}
					...
					<section id="headings" class="menu-anchor">
						<div class="panel panel-default">
							<div class="panel-heading">
								...
								<div class="pull-right">
									<a href="index.php?page=page/services/add" class="btn btn-cyanide"><i class="fa fa-tags"></i> Hizmet Ekle</a>
									</div>
									<div class="clearfix"></div>
							</div>
							<div class="panel-body">
								<div class="panel panel-default">
										...
										<table class="table table-striped table-hover table-bordered">
											...
											<tbody id="sortable" data-page="page/services/sort">
											{if $products eq TRUE}
											{foreach from=$products item=don}
												<tr id="item-{$don.id}">
													<td class="text-center"><button type="button" data-name="{$don.seo_title}" data-database="page/services/status" data-id="{$don.id}" data-status="{$don.status}" class="btn {if $don.status eq 0}btn-primary{else}btn-danger{/if} btn-xs status"><i class="fa fa-star" aria-hidden="true"></i></button></td>
													<td class="text-center"><button type="button" class="btn btn-warning btn-xs "><i class="fa fa-arrows" aria-hidden="true"></i></button></td>
													<td>{$don.id}</td>
													<td>{$don.seo_title}</td>
													<td>
														<a href="index.php?page=page/services/edit&id={$don.id}"><span class="label label-info"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Düzenle</span></a>
														<a class="divtikla" href="index.php?page=page/services/delete&id={$don.id}"><span class="label label-danger"><i class="fa fa-times" aria-hidden="true"></i> Sil</span></a>
													</td>
												</tr>
											{/foreach}
											{else}
		...
		{if $categorys eq 1}	
			...
			<div id="kategoriekle" class="modal fade" tabindex="-1" data-replace="true" data-width="760" style="display: none;">
				<form class="popupkaydet" action="index.php?page=page/services/category/add" method="post" enctype="multipart/form-data">
				<input type="hidden" name="{$tokenID}" value="{$token}" />
