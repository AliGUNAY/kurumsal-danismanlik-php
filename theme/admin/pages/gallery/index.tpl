									...
									<table class="table table-striped">
										<thead> 
											...
											{foreach from=$products key=k item=don}
												<tr id="item-{$don.id}">
													<td class="text-center"><button type="button" data-name="{$don.seo_title}" data-database="page/gallery/status" data-id="{$don.id}" data-status="{$don.status}" class="btn {if $don.status eq 0}btn-primary{else}btn-danger{/if} btn-xs status"><i class="fa fa-star" aria-hidden="true"></i></button></td>
													<td class="text-center"><button type="button" class="btn btn-warning btn-xs "><i class="fa fa-arrows" aria-hidden="true"></i></button></td>
													<td>{$k}</td>
													<td><img style="    width: 200px;" src="../upload/photo/{if !empty($don.photo)}{$don.photo}{else}no-banner.jpg{/if}" /></td>
													<td>{$don.seo_title}</td>
													<td>
														<a href="index.php?page=page/gallery/edit&id={$don.id}"><span class="label label-info"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Düzenle</span></a>
														<a class="divtikla" href="index.php?page=page/gallery/delete&id={$don.id}"><span class="label label-danger"><i class="fa fa-times" aria-hidden="true"></i> Sil</span></a>
													</td>
												</tr>
											{/foreach}
											{else}
